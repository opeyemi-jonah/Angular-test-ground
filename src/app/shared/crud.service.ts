...
export class CrudService {
  ...
  getUsers(): Observable<User> {
    return this.httpClient.get<User>(this.endpoint + '/users').pipe(retry(1),catchError(this.processError));
  }

  getSingleUser(id: any) : Observable<User> {
    return this.httpClient.get<User>(this.endpoint + '/users/' + id).pipe(retry(1), catchError(this.processError))
  }
  ...
  updateUser(id: any, data: any): Observable <User> {
    return this.httpClient.put<User> (
      this.endpoint + 'users/'+ id, JSON.stringify(data), this.httpHeader)
      .pipe(retry(1), catchError(this.processError))
